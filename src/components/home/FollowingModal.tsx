...
import { getFollowing, unfollowUserById } from "@/app/api/friends";
import { useAppDispatch, useAppSelector } from "@/lib/hook";
import { setFollowingModal } from "@/redux/slices/modals";

const FollowingModal = () => {
  const dispatch = useAppDispatch();
  const modal = useAppSelector((state) => state.modals.followingModal);
          ...
          {following.length > 0 &&
            following.map((user, index) => (
              <div className="flex items-center justify-between w-60">
                ...
                <button
                  className="px-2 py-1 text-sm text-white bg-primary-700 rounded-md min-w-[80px] flex justify-center items-center hover:scale-95 active:scale-100"
